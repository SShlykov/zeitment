...
func (r *repository) Create(ctx context.Context, pageDto *models.Page) (string, error) {
	query := `INSERT INTO` + " " + tableName + ` (` + insertItems() + `) VALUES ($1, $2, $3, $4, $5) ` +
		Returning + columnID
	args := []interface{}{pageDto.Title, pageDto.Text, pageDto.ChapterID, pageDto.IsPublic, pageDto.MapParamsID}

	q := postgres.Query{Name: "PageRepository.Create", Raw: query}
...
func (r *repository) FindByID(ctx context.Context, id string) (*models.Page, error) {
	query := querybuilder.SelectWhere(allItems, tableName, columnID)
	q := postgres.Query{Name: "PageRepository.FindByID", Raw: query}
...
func (r *repository) Update(ctx context.Context, id string, updPage *models.Page) (*models.Page, error) {
	query := "Update " + tableName + " SET " +
		querybuilder.ParamsToQuery(", ", columnTitle, columnText, columnChapterID, columnIsPublic, columnMapParamsID) +
	...
	q := postgres.Query{Name: "PageRepository.Update", Raw: query}
...
func (r *repository) Delete(ctx context.Context, id string) (*models.Page, error) {
	...
	q := postgres.Query{Name: "PageRepository.Delete", Raw: query}
	row := r.db.DB().QueryRowContext(ctx, q, id)
...
func (r *repository) List(ctx context.Context) ([]models.Page, error) {
	...
	q := postgres.Query{Name: "PageRepository.List", Raw: query}
...
func (r *repository) GetPagesByChapterID(ctx context.Context, chapterID string) ([]models.Page, error) {
	query := querybuilder.SelectWhere(allItems, tableName, columnChapterID) + ` AND ` + columnDeletedAt + ` IS NULL`

	q := postgres.Query{Name: "PageRepository.GetPagesByChapterID", Raw: query}
