...
func TestBookEventController_TogglePublic(t *testing.T) {
	t.Parallel()
	...
	service.EXPECT().TogglePublic(gomock.Any(), gomock.Any()).Return(bookEvent, nil)

	e := echo.New()
	req := httptest.NewRequest(http.MethodPost, v1.BookEventsPath+v1.ToggleSubPath, strings.NewReader(fixture.RequestPageOptions))
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec := httptest.NewRecorder()
	c := e.NewContext(req, rec)

	bc := NewBookEventController(service, fixture.Metrics, fixture.Logger, fixture.Context)
	err := bc.TogglePublic(c)
	if err != nil {
		return
	}
